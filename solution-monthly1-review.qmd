---
title: "Solutions: Core functions"
---
...
```{r}
bo_title_dist_cnt <- boxoffice |> 
  group_by(title, distributor) |> 
  summarize(days_shown = n()) |> 
  arrange(days_shown |> desc())

bo_title_dist_cnt

# this uses the shortcut count() to take the list above and recount
# the titles to see which had more than one distributor
bo_title_dist_cnt |> 
  count(title, sort = 2)
```
