...
`list.files()` (you guessed it) lists files in a directory we specify. In our case it is a smidge complicated because we want specific files from among all the files in the `data-raw` folder, so we'll find them using a regular expression pattern.
