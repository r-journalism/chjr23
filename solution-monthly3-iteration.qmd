...
This lesson covers some tools for iteration ... to repeat the same same action on multiple objects. The concepts used here are covered in more detail in the [Functions](https://r4ds.hadley.nz/functions) and [Iteration](https://r4ds.hadley.nz/iteration) chapters of R for Data Science.

Some of the tasks we'll accomplish:

- Import and bind multiple files
- Create multiple plots from once data source
- Summarize the same function across multiple columns
- Filter across multiple columns

We won't get through it all in 45 mins, but hopefully the explanations help.

## New packages

This lesson uses a package called clipr that probably need to install. Uncomment the line, run it, then recomment it.
...
We have weather files from six different states. We could import them each individually and then use `bind_rows()` to put them together. That's a lot even for just six files ... what if there were dozens?
...
- Create a list of the files we want
- Use `map()` to apply the read_csv function to each file
- Bind them all together (which we'll pipe into after the map)

### Get a list of files

`list.files()` (you guessed it) lists files in a directory we specify. Our use case is a smidge complicated because we want specific files from among all the files in data-raw, so we'll find them using a regular expression pattern.

Our pattern is looking for file names that "start with two letters and then .csv". This matches our two-letter state files for our weather data. You can how more regex in this [Regular Expressions NICAR lesson](https://bit.ly/nicar-regex).
...
- We'll pull values from a column into a vector
- I'll demonstrate multi-line editing

#### Copy the list of cities

We'll use this next code to get a list of all our cities for the renaming-cites chunk below.
...
It will help to set up the next chunk first, but once we do we'll come back to thsi.

#### Build the mutate

We want to a column that has a nice version of the city from our "name" field. Since some city names are two words and there isn't a common separator we'll have to do this manually, but I want to show some neat editing tricks that make this easier. We'll be working back 'n' forth between the next two code chunks.

1. I'll first build the `case_match()`
2. I'll go the city-names chunk below to copy the city names
3. I'll come back to this chunk an paste them in
4. I'll use multi-line editing to set up the list for the clean names. We'll use *option+click+drag* (Mac) or *alt+click+drag* (PC) to highlight the station names and then edit all the lines at once.
...
What's might be new here is to build the state we use `str_sub()` to pull characters from the `name` field based on their position. To get it we count backward from the end of the string since it ends with "XX US" where XX is the state. We start with the 5th character from end, and then stop at the 4th character from the end.
...
## Chart average highs, lows

This next section isn't new, but we have to do it before we can show more new stuff.
...
## Iterate over cities to build charts
...
3. Go to the next chunk where we map the function and return it while watching your Files pane. You should see the images for each plot appear.
...
- Our first across creates temporary function "x" and passes it the `sum()` function with the needed "na.rm = T" argument.
